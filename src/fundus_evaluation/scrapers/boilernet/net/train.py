...
def get_dataset(dataset_file, batch_size, repeat=True):
    def _read_example(example):
        desc = {"doc_feature_list": tf.io.VarLenFeature(tf.int64), "doc_label_list": tf.io.VarLenFeature(tf.int64)}
        _, seq_features = tf.io.parse_single_sequence_example(example, sequence_features=desc)
        return tf.sparse.to_dense(seq_features["doc_feature_list"]), tf.sparse.to_dense(seq_features["doc_label_list"])

    buffer_size = 10 * batch_size
    dataset = (
        tf.data.TFRecordDataset([dataset_file])
...
def main():
    ...
    ap.add_argument("-u", "--hidden_units", type=int, default=256, help="The number of hidden LSTM units")
    ...
    ap.add_argument(
        "--interval", type=int, default=5, help="Calculate metrics and save the model after this many epochs"
    )
    ...
    clf.train(
        ...
        class_weights,
        dev_dataset,
        info.get("num_dev_examples"),
        test_dataset,
        info.get("num_test_examples"),
        args.interval,
    )
