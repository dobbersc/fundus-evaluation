...
Original from:
    - https://github.com/chatnoir-eu/web-content-extraction-benchmark/blob/221b6503d66bf4faa378e6ae3c3f63ee01d584c6/src/extraction_benchmark/extractors/bte.py
    - https://github.com/dalab/web2text/blob/0f9c7b787ff125ce5190784e741c5b453ddf0560/other_frameworks/bte/bte.py
"""
...
def preclean(html_text):
    ...
    # strip all but body
    cleaned_text = re.compile("^.*<body(\s+[^>]*)?>", re.S | re.I).sub("", html_text)
    cleaned_text = re.compile("</body>.*$", re.S | re.I).sub("", cleaned_text)

    # strip scripts
    cleaned_text = re.compile("<script(\s+[^>]*)?>(.|\s)*?</script>", re.S | re.I).sub(
        "<script></script>", cleaned_text
    )

    # strip styles
    cleaned_text = re.compile("<style(\s+[^>]*)?>(.|\s)*?</style>", re.S | re.I).sub("<style></style>", cleaned_text)
